...
new_w = width
new_h = height
if largestSideSize > 0 and width > height:
    new_h = largestSideSize
    new_w = round(width * new_h / height)
elif largestSideSize > 0:
...
cv2.rectangle(mask, bg_rect[:2], bg_rect[2:4], color=2, thickness=bg_w * 3)
...
print(mask_dict[0], mask_dict[255] * 1.5)
if mask_dict[0] > mask_dict[255] * 1.5:
