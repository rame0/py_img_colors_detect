...
class App():
    ...
    def run(self):
        ...
        sub_width = round(width * .01)
        sub_height = round(height * .01)

        self.mask = np.zeros(self.img.shape[:2], dtype=np.uint8)  # mask initialized to PR_BG

        # self.rect = (5, 5, width - 5, height - 5)
        self.rect = (sub_width, sub_height, width - sub_width, height - sub_height)
        cv.rectangle(self.img, self.rect[:2], self.rect[2:4], self.BLACK, 4)
        self.rect_or_mask = 0
